  ...
  const rows = items
    .map(
      (g) =>
        `<li>${E(g.summary ?? "")}${
          g.files
            ? ` <span class="dim">(${n(g.files)} file${g.files === 1 ? "" : "s"})</span>`
            : ""
        }</li>`,
    )
    .join("");
...
export function grammarStatus(g, name) {
  ...
  const facts = [
    [
      "evidence",
      flag(
        g.evidence_freshness === "current",
        "current",
        g.evidence_freshness ?? "—",
      ),
    ],
...
<div class="facts">${facts
    .map(([k, v]) => `<div><span class="k">${E(k)}</span>${v}</div>`)
    .join("")}</div>

<div class="facts counts">${counts
    .map(([k, v]) => `<div><span class="k">${E(k)}</span><b>${n(v)}</b></div>`)
    .join("")}</div>

<p class="dim note">Reference-parser capabilities: ${caps
    .map(([k, v]) => `${E(k)} ${flag(v, "yes", "no")}`)
    .join(
      " · ",
...
<dl class="declared">${declared
    .map(
      ([file, on, what]) =>
        `<dt class="${on ? "on" : "off"}"><code>${E(file)}</code>${
          on
            ? '<span class="flag on">declared</span>'
            : '<span class="flag off">none</span>'
        }</dt><dd>${E(what)}</dd>`,
    )
    .join("")}</dl>
...
export function statusOverview(status) {
  ...
  const rows = names
    .map((name) => {
      ...
      const c = g.corpora?.[0];
      const gaps = (g.corpora ?? []).reduce(
        (t, x) => t + (x.grammar_gaps ?? 0),
        0,
      );
