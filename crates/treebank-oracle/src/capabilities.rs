...
pub(crate) fn get(name: LangName) -> Capabilities {
    match name {
        ...
        LangName::Sql => Capabilities {
            // SQLite parses to a VDBE program, not to a tree with source
            // positions: `EXPLAIN` reports opcodes, and the parse tree is
            // freed inside sqlite3_prepare. libpg_query would give one --
            // it returns postgres's own parse tree as JSON with node
            // locations -- and it is the same build this language's third
            // validity oracle needs.
            spans: None,
            // sqlformat and pg_format are the candidates and neither is
            // installed; SQLite ships no formatter of its own.
            reformat: None,
            // Nothing renders SQL back from sqlite's parse, for the reason
            // above: the tree is gone before the statement finishes
            // preparing.
            unparse: None,
        },
    }
