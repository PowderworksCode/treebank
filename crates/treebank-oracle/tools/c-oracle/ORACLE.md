# The C and C++ validity oracle

*Recovered from the era when this repository redistributed a patched
tree-sitter-c, and kept because the argument in it is about clang and not
about which grammar was being measured. It is now also the C++ oracle: the
same program, the same categorical rule, `-x c++ -std=gnu++17` instead of
`-x c -std=gnu17`.*
...
`crates/treebank-oracle/tools/c-oracle` — a small C program against **libclang**, the analogue of
...
compiler driver. `build.sh` pins the libclang it builds against, and the pin is
load-bearing for the same reason `generate_cli` is: clang's error-recovery
...
- `crates/treebank-oracle/tools/c-oracle` emits all three counts, and `lang/c.rs` writes
