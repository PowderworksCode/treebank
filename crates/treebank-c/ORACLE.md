...
| ├─ oracle: **valid** → grammar gap | **5,629** |
| ├─ oracle: **invalid** → corpus noise | **302** |
| └─ oracle: **indeterminate** → cannot say | **12,006** |

Indeterminate outnumbers valid **2.1 : 1**. 95%+ of those files carry an
unresolved `#include`, and the single most common missing header is
`config.h`, which is generated by `./configure` and which no static include
path can supply.
...
1. **The oracle has real rejection power.** 302 corpus files were positively
...
2. **The 5,629 gap files are not weakened by the 12,006.** Each of them is a
...
The one lever that would move the indeterminate mass is `config.h`. Supplying
an empty stub for unresolved includes would convert much of it — no upstream
code executed — at the cost of a corpus whose macros are known-absent rather
than merely missing. That is a deliberate escalation with its own honesty
question, and it is not taken here.
