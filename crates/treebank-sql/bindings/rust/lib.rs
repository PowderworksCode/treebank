//! treebank-sql — the treebank SQL grammar.
//!
//! Beyond the usual tree-sitter crate surface this exposes [`ROLES`], the
//! grammar's `roles.json`: the facet-tier membership (`_binding`,
//! `_scope`, `_clause`, `_modifier`, …) that cannot live in the parse
//! table — `_modifier` among them, which SQL partitions per construct and
//! this grammar therefore declares as a demotion (DESIGN.md §3.1.1) —
...
mod tests {
    ...
    fn ships_its_manifests() {
        ...
        assert_eq!(ledger["language"].as_str(), Some("sql"));
    }
}
