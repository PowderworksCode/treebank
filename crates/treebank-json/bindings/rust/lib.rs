//! treebank-json — the treebank JSON grammar (strict RFC 8259).
//!
//! Beyond the usual tree-sitter crate surface this exposes [`ROLES`], the
//! grammar's `roles.json`: the facet-tier membership that cannot live in
//! the parse table, plus the nodes deliberately outside the vocabulary and
//! why. It travels inside the published crate so a consumer never has to
//! fetch it separately — the table-tier roles are already queryable from
//! the parser itself.
...
/// The grammar's evidence file (`ledger.toml`): the dialect argument, the
/// pinned oracle, corpus and sweep numbers, and declared deviations.
///
...
mod tests {
    ...
    fn ships_its_manifests() {
        ...
        assert_eq!(ledger["language"].as_str(), Some("json"));
    }
}
