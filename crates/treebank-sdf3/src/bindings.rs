...
pub fn emit(module: &Module, names: &Names) -> Result<Option<Emitted>> {
            ...
            match a {
                ...
                Attr::Binds(b) => {
                    ...
                    let target = match b.target {
                        BindTarget::Enclosing => "enclosing",
                        BindTarget::Module => "module",
                    };
                    definitions.push(json!({
                        "node": node, "field": b.label, "name": token,
                        "scope": target, "kind": kind, "from": p.display()
                    }));
                    ...
                    if b.target == BindTarget::Module {
                        findings.push(Finding {
                            kind: Kind::Deviation,
                            what: format!(
                                "{}: `binds({} -> module)`: the locals query dialect cannot name the module scope, so the pattern binds at the nearest scope; bindings.json carries the target",
                                p.display(),
                                b.label
                            ),
                        });
                        scm_defs.push(format!("; bindings.json: scope = module. The dialect has no way to say it.\n{pat})"));
                    } else if is_scope {
                    ...
                    findings.push(Finding {
                        kind: Kind::Extension,
                        what: format!(
                            "{}: `binds({} -> {target} as {kind})` (not SDF3): the `{token}` under `{}` of `{node}` is bound in the {target} scope",
                            p.display(),
                            b.label,
                            b.label
                        ),
    ...
    let json = json!({
        "note": "GENERATED by treebank-sdf3 from the scope/binds/refers attributes of the module. A definition's `scope` is `enclosing` (the nearest scope node that is a proper ancestor of the binding node) or `module` (the outermost). A reference is any `name` node no definition claims. tools/bindings_check.py holds this to CPython's symtable.",
        "scopes": scopes,
