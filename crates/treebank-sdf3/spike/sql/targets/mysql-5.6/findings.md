## WIDENING -- tree-sitter accepts more than SDF3 here (4)
...
## EXTENSION -- a treebank addition outside SDF3 was used (54)
...
## MAPPED -- lowered exactly (61)

- imports [mysql/base, mysql/query-cache, mysql/no-cache] merged additively by the loader: an imported sort gains this module's productions, nothing is overridden -- where tree-sitter's `extends` would flatten and override
...
- a `{Elem Sep}*` list expanded to seq/repeat; the expansion has no name in grammar.json
- Exp.Call at priority level 8 became PREC at that level
- Exp.Neg at priority level 7 became PREC at that level
- Exp.Mul at priority level 6 became PREC_LEFT at that level
- Exp.Add at priority level 5 became PREC_LEFT at that level
- Exp.Sub at priority level 5 became PREC_LEFT at that level
- Exp.Eq at priority level 4 became PREC_LEFT at that level
- Exp.Lt at priority level 4 became PREC_LEFT at that level
- Exp.Gt at priority level 4 became PREC_LEFT at that level
- Exp.Like at priority level 4 became PREC_LEFT at that level
- Exp.Not at priority level 3 became PREC at that level
- Exp.And at priority level 2 became PREC_LEFT at that level
- Exp.Or at priority level 1 became PREC_LEFT at that level
...
- roles.json: 9 of 22 table-tier terms are supertypes, 2 facet(s), 55 named node(s), 9 uncategorised (vocabulary 0.1.0)
