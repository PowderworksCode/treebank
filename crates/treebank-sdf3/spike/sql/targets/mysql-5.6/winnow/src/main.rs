// GENERATED from mysql/5.6.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 6 >= min && block != Some(6) { let cp = save(i); match t_32(i, &left, start) { Ok(n) => { left = n; block = if false { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 5 >= min && block != Some(5) { let cp = save(i); match t_33(i, &left, start) { Ok(n) => { left = n; block = if false { Some(5) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 5 >= min && block != Some(5) { let cp = save(i); match t_34(i, &left, start) { Ok(n) => { left = n; block = if false { Some(5) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_35(i, &left, start) { Ok(n) => { left = n; block = if true { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_36(i, &left, start) { Ok(n) => { left = n; block = if true { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_37(i, &left, start) { Ok(n) => { left = n; block = if true { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_38(i, &left, start) { Ok(n) => { left = n; block = if true { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_40(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 1 >= min && block != Some(1) { let cp = save(i); match t_41(i, &left, start) { Ok(n) => { left = n; block = if false { Some(1) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
