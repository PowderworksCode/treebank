...
- `keyword = case-insensitive` (not SDF3's template options; its productive form is the `'kw'` literal): 37 keywords became `token(prec(1, /[sS][eE]../))` rules aliased to their spelling and reserved by symbol; the tree and the printer show the template's spelling
...
- a `{Elem Sep}*` list expanded to seq/repeat; the expansion has no name in grammar.json
- Exp.Call at priority level 11 became PREC at that level
- Exp.Neg at priority level 10 became PREC at that level
- Exp.Mul at priority level 9 became PREC_LEFT at that level
- Exp.Add at priority level 8 became PREC_LEFT at that level
- Exp.Sub at priority level 8 became PREC_LEFT at that level
- Exp.Eq at priority level 7 became PREC_LEFT at that level
- Exp.Lt at priority level 7 became PREC_LEFT at that level
- Exp.Gt at priority level 7 became PREC_LEFT at that level
- Exp.Like at priority level 7 became PREC_LEFT at that level
- Exp.Not at priority level 6 became PREC at that level
- Exp.And at priority level 5 became PREC_LEFT at that level
- Exp.Or at priority level 4 became PREC_LEFT at that level
...
- `NAME = keyword {reject}` became `word: name` plus reserved.global = [AND, AS, ASC, BY, CREATE, DELETE, DESC, DROP, DUPLICATE, FROM, IGNORE, INSERT, INT, INTO, KEY, LIKE, LIMIT, NOT, NULL, OFFSET, ON, OR, ORDER, OVER, PARTITION, REPLACE, SELECT, SET, SQL_CACHE, SQL_NO_CACHE, TABLE, TEXT, UPDATE, VALUES, VARCHAR, WHERE, WITH]
...
- roles.json: 9 of 22 table-tier terms are supertypes, 2 facet(s), 59 named node(s), 11 uncategorised (vocabulary 0.1.0)
