// GENERATED from mariadb/10.11.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn lx_name(i: &mut In) -> ModalResult<Node> {
    ...
    let end = pos(i);
    let text = &i.state.src[start..end];
    const REJECT: &[&str] = &["AND", "AS", "ASC", "BY", "CREATE", "DELETE", "DESC", "DROP", "DUPLICATE", "FROM", "IGNORE", "INSERT", "INT", "INTO", "KEY", "LIKE", "LIMIT", "NOT", "NULL", "OFFSET", "ON", "OR", "ORDER", "OVER", "PARTITION", "REPLACE", "SELECT", "SET", "SQL_CACHE", "SQL_NO_CACHE", "TABLE", "TEXT", "UPDATE", "VALUES", "VARCHAR", "WHERE", "WITH"];
    if REJECT.iter().any(|k| eq_ci(k, text)) { return Err(bt()); }
    token_end(i, end);
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 9 >= min && block != Some(9) { let cp = save(i); match t_32(i, &left, start) { Ok(n) => { left = n; block = if false { Some(9) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_33(i, &left, start) { Ok(n) => { left = n; block = if false { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_34(i, &left, start) { Ok(n) => { left = n; block = if false { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_35(i, &left, start) { Ok(n) => { left = n; block = if true { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_36(i, &left, start) { Ok(n) => { left = n; block = if true { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_37(i, &left, start) { Ok(n) => { left = n; block = if true { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_38(i, &left, start) { Ok(n) => { left = n; block = if true { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 5 >= min && block != Some(5) { let cp = save(i); match t_40(i, &left, start) { Ok(n) => { left = n; block = if false { Some(5) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_41(i, &left, start) { Ok(n) => { left = n; block = if false { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
