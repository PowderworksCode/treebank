// GENERATED from mysql/5.7.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn lx_name(i: &mut In) -> ModalResult<Node> {
    ...
    let end = pos(i);
    let text = &i.state.src[start..end];
    const REJECT: &[&str] = &["AND", "AS", "ASC", "BY", "CREATE", "DELETE", "DESC", "DROP", "DUPLICATE", "FROM", "IGNORE", "INSERT", "INT", "INTO", "KEY", "LIKE", "LIMIT", "NOT", "NULL", "OFFSET", "ON", "OR", "ORDER", "REPLACE", "SELECT", "SET", "SQL_CACHE", "SQL_NO_CACHE", "TABLE", "TEXT", "UPDATE", "VALUES", "VARCHAR", "WHERE"];
    if REJECT.iter().any(|k| eq_ci(k, text)) { return Err(bt()); }
    token_end(i, end);
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 8 >= min && block != Some(8) { let cp = save(i); match t_32(i, &left, start) { Ok(n) => { left = n; block = if false { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_33(i, &left, start) { Ok(n) => { left = n; block = if false { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 7 >= min && block != Some(7) { let cp = save(i); match t_34(i, &left, start) { Ok(n) => { left = n; block = if false { Some(7) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 6 >= min && block != Some(6) { let cp = save(i); match t_35(i, &left, start) { Ok(n) => { left = n; block = if true { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 6 >= min && block != Some(6) { let cp = save(i); match t_36(i, &left, start) { Ok(n) => { left = n; block = if true { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 6 >= min && block != Some(6) { let cp = save(i); match t_37(i, &left, start) { Ok(n) => { left = n; block = if true { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 6 >= min && block != Some(6) { let cp = save(i); match t_38(i, &left, start) { Ok(n) => { left = n; block = if true { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_40(i, &left, start) { Ok(n) => { left = n; block = if false { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 3 >= min && block != Some(3) { let cp = save(i); match t_41(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_51(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_52(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
...
fn c_49_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("SQL_CACHE")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "cache", start, end, reach: end, children: ch })
}
