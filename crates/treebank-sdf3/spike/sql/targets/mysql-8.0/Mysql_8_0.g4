// GENERATED from mysql/8.0.sdf3 by treebank-sdf3's ANTLR backend. Python3 target.
grammar Mysql_8_0;
...
select
    : 'SELECT' (hints+=no_cache)* items+=item (',' items+=item)* (from_=from)? (where_=where)? (order_=order_by)? (limit_=limit)? (offset_=offset)?
    ;
