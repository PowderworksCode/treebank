// GENERATED from mysql/8.0.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn layout(i: &mut In) -> ModalResult<()> {
    ...
    loop {
        ...
        { let s = pos(i); if let Ok(()) = run(((literal("#"), star(none_of(|c: char| matches!(c, '\n' | '\r'))),).void()).void(), i) { let e = pos(i); i.state.comments.insert(s, (e, "comment_2")); progressed = true; continue; } }
        if !progressed { break; }
    }
    let after = pos(i);
...
fn lxb_backtick(i: &mut In) -> ModalResult<()> { run((literal("`"), star(none_of(|c: char| matches!(c, '`'))), literal("`"),).void(), i) }
fn lx_backtick(i: &mut In) -> ModalResult<Node> {
    let start = pos(i);
    ...
    token_end(i, end);
    Ok(Node::leaf("backtick", start, end))
}
fn lxb_dqstring(i: &mut In) -> ModalResult<()> { run((literal("\""), star(none_of(|c: char| matches!(c, '"'))), literal("\""),).void(), i) }
fn lx_dqstring(i: &mut In) -> ModalResult<Node> {
    let start = pos(i);
    i.state.furthest = i.state.furthest.max(start);
    lxb_dqstring(i)?;
    let end = pos(i);
    ...
    if REJECT.iter().any(|k| eq_ci(k, text)) { return Err(bt()); }
    token_end(i, end);
    Ok(Node::leaf("dqstring", start, end))
}
...
fn lx_name(i: &mut In) -> ModalResult<Node> {
    ...
    let end = pos(i);
    let text = &i.state.src[start..end];
    const REJECT: &[&str] = &["AND", "AS", "ASC", "BY", "CREATE", "DELETE", "DESC", "DROP", "DUPLICATE", "FROM", "IGNORE", "INSERT", "INT", "INTO", "KEY", "LIKE", "LIMIT", "NOT", "NULL", "OFFSET", "ON", "OR", "ORDER", "OVER", "PARTITION", "REPLACE", "SELECT", "SET", "SQL_NO_CACHE", "TABLE", "TEXT", "UPDATE", "VALUES", "VARCHAR", "WHERE", "WITH"];
    if REJECT.iter().any(|k| eq_ci(k, text)) { return Err(bt()); }
    token_end(i, end);
    Ok(Node::leaf("name", start, end))
}
fn lxb_string(i: &mut In) -> ModalResult<()> { run(alt(((literal("'"), star(alt(((literal("''"),).void(), (none_of(|c: char| matches!(c, '\'')),).void(),))), literal("'"),).void(), (lxb_dqstring,).void(),)), i) }
...
fn c_13_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { let mut v: Vec<Node> = Vec::new(); #[allow(unused_mut, unused_variables)] let mut col0: Option<usize> = None; loop { let cp = save(i); if layout(i).is_err() { restore(i, &cp); break; }  match (|i: &mut In| -> ModalResult<Vec<Node>> { Ok(vec![r_inserthint(i)?]) })(i) { Ok(ns) => v.extend(ns), Err(_) => { restore(i, &cp); break; } } } if v.len() < 0 { return Err(bt()); } v };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("hints"), n)); } }
    layout(i)?;
    { let s = pos(i);
...
fn c_48_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("REPLACE")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    layout(i)?;
    { let s = pos(i);
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "replace", start, end, reach: end, children: ch })
}
...
fn c_2_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { let mut v: Vec<Node> = Vec::new(); #[allow(unused_mut, unused_variables)] let mut col0: Option<usize> = None; loop { let cp = save(i); if layout(i).is_err() { restore(i, &cp); break; }  match (|i: &mut In| -> ModalResult<Vec<Node>> { Ok(vec![r_selecthint(i)?]) })(i) { Ok(ns) => v.extend(ns), Err(_) => { restore(i, &cp); break; } } } if v.len() < 0 { return Err(bt()); } v };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("hints"), n)); } }
    layout(i)?;
    { let s = pos(i);
...
fn c_45_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![lx_backtick(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 10 >= min && block != Some(10) { let cp = save(i); match t_32(i, &left, start) { Ok(n) => { left = n; block = if false { Some(10) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 9 >= min && block != Some(9) { let cp = save(i); match t_33(i, &left, start) { Ok(n) => { left = n; block = if false { Some(9) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 9 >= min && block != Some(9) { let cp = save(i); match t_34(i, &left, start) { Ok(n) => { left = n; block = if false { Some(9) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_35(i, &left, start) { Ok(n) => { left = n; block = if true { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_36(i, &left, start) { Ok(n) => { left = n; block = if true { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_37(i, &left, start) { Ok(n) => { left = n; block = if true { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 8 >= min && block != Some(8) { let cp = save(i); match t_38(i, &left, start) { Ok(n) => { left = n; block = if true { Some(8) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 6 >= min && block != Some(6) { let cp = save(i); match t_40(i, &left, start) { Ok(n) => { left = n; block = if false { Some(6) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 5 >= min && block != Some(5) { let cp = save(i); match t_41(i, &left, start) { Ok(n) => { left = n; block = if false { Some(5) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_50(i, &left, start) { Ok(n) => { left = n; block = if false { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 4 >= min && block != Some(4) { let cp = save(i); match t_51(i, &left, start) { Ok(n) => { left = n; block = if false { Some(4) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_53(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
...
fn c_31_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![r_exp_prec(i, 11)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
...
fn t_33_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![r_exp_prec(i, 10)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("right"), n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
...
fn t_50(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_50_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_50_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn t_53(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_53_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_53_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn r_inserthint(i: &mut In) -> ModalResult<Node> { r_inserthint_prec(i, 0) }
fn r_inserthint_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    layout(i)?;
    let start = pos(i);
    let cp = save(i);
    // Longest match among the primaries: ordered choice would let an
    ...
    // first in prefer/source order.
    let mut best: Option<(usize, Node, Cp)> = None;
    restore(i, &cp); if let Ok(n) = c_46(i) { let e = pos(i); if best.as_ref().map_or(true, |b| e > b.0) { best = Some((e, n, save(i))); } }
    let mut left: Node = match best { Some((_, n, at)) => { restore(i, &at); n } None => { restore(i, &cp); return Err(bt()); } };
    let _ = min;
    Ok(left)
}
fn c_46(i: &mut In) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = c_46_body(i);
    i.state.offside.truncate(guard);
    r
}
fn c_46_body(i: &mut In) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("IGNORE")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "ignore", start, end, reach: end, children: ch })
}
...
fn c_47_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("DUPLICATE")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("KEY")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    layout(i)?;
    { let s = pos(i);
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "on_duplicate_key", start, end, reach: end, children: ch })
}
fn r_selecthint(i: &mut In) -> ModalResult<Node> { r_selecthint_prec(i, 0) }
fn r_selecthint_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    layout(i)?;
    let start = pos(i);
    let cp = save(i);
    // Longest match among the primaries: ordered choice would let an
...
fn c_49_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal(Caseless("SQL_NO_CACHE")), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "no_cache", start, end, reach: end, children: ch })
}
