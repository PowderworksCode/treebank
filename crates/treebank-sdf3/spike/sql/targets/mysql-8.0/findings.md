...
- 3 independent priority chains; tree-sitter precedence is one global order, so their levels are numbered together and may interact
...
- 11 named node(s) outside the vocabulary, ledgered as uncategorised: [backtick, col_def, cte, int, item, name, order, partition, script, select, string]

## EXTENSION -- a treebank addition outside SDF3 was used (61)
...
- `keyword = case-insensitive` (not SDF3's template options; its productive form is the `'kw'` literal): 36 keywords became `token(prec(1, /[sS][eE]../))` rules aliased to their spelling and reserved by symbol; the tree and the printer show the template's spelling
...
## MAPPED -- lowered exactly (68)

- imports [mysql/5.7, sql/cte, sql/window] merged additively by the loader: an imported sort gains this module's productions, nothing is overridden -- where tree-sitter's `extends` would flatten and override
...
- a `{Elem Sep}*` list expanded to seq/repeat; the expansion has no name in grammar.json
- Exp.Call at priority level 12 became PREC at that level
- Exp.Neg at priority level 11 became PREC at that level
- Exp.Mul at priority level 10 became PREC_LEFT at that level
- Exp.Add at priority level 9 became PREC_LEFT at that level
- Exp.Sub at priority level 9 became PREC_LEFT at that level
- Exp.Eq at priority level 8 became PREC_LEFT at that level
- Exp.Lt at priority level 8 became PREC_LEFT at that level
- Exp.Gt at priority level 8 became PREC_LEFT at that level
- Exp.Like at priority level 8 became PREC_LEFT at that level
- Exp.Not at priority level 7 became PREC at that level
- Exp.And at priority level 6 became PREC_LEFT at that level
- Exp.Or at priority level 5 became PREC_LEFT at that level
- Exp.Arrow at priority level 4 became PREC_LEFT at that level
- Exp.ArrowText at priority level 4 became PREC_LEFT at that level
- Exp.Over at priority level 2 became PREC at that level
...
- sort SelectHint has the single constructor NoCache; collapsed to the named rule `no_cache`
- a `{Elem Sep}+` list expanded to seq/repeat; the expansion has no name in grammar.json
...
- `NAME = keyword {reject}` became `word: name` plus reserved.global = [AND, AS, ASC, BY, CREATE, DELETE, DESC, DROP, DUPLICATE, FROM, IGNORE, INSERT, INT, INTO, KEY, LIKE, LIMIT, NOT, NULL, OFFSET, ON, OR, ORDER, OVER, PARTITION, REPLACE, SELECT, SET, SQL_NO_CACHE, TABLE, TEXT, UPDATE, VALUES, VARCHAR, WHERE, WITH]
...
- `_modifier` threaded as a supertype over [_dir, no_cache, ignore]; every reference to a member now goes through it, and the tree is unchanged; [_dir] are sorts, not terms, and were flattened into it
- `_clause` is a facet: [as, bare, from, limit, offset, on_duplicate_key, order_by, where, with] listed in roles.json; [_alias] left the supertypes list (hidden rules still), since a supertype must be a table-tier term
