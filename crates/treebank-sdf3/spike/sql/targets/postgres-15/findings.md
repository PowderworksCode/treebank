...
## EXTENSION -- a treebank addition outside SDF3 was used (66)
...
- Stmt.Merge: placeholder label `target` became a field (not SDF3)
- Stmt.Merge: placeholder label `source` became a field (not SDF3)
...
- When.NotMatched: placeholder label `columns` became a field (not SDF3)
- When.NotMatched: placeholder label `values` became a field (not SDF3)
- `keyword = case-insensitive` (not SDF3's template options; its productive form is the `'kw'` literal): 43 keywords became `token(prec(1, /[sS][eE]../))` rules aliased to their spelling and reserved by symbol; the tree and the printer show the template's spelling
- `vocabulary` section (not SDF3): 10 terms of treebank's vocabulary 0.1.0 bound to this module's sorts and constructors
...
## MAPPED -- lowered exactly (72)

- imports [postgres/12, postgres/merge] merged additively by the loader: an imported sort gains this module's productions, nothing is overridden -- where tree-sitter's `extends` would flatten and override
...
- `NAME = keyword {reject}` became `word: name` plus reserved.global = [AND, AS, ASC, BY, CONFLICT, CREATE, DELETE, DESC, DO, DROP, FROM, ILIKE, INSERT, INT, INTO, LIKE, LIMIT, MATCHED, MERGE, NOT, NOTHING, NULL, OFFSET, OIDS, ON, OR, ORDER, OVER, PARTITION, RETURNING, SELECT, SET, TABLE, TEXT, THEN, UPDATE, USING, VALUES, VARCHAR, WHEN, WHERE, WITH, WITHOUT]
...
- `_modifier` threaded as a supertype over [_dir, without_oids]; every reference to a member now goes through it, and the tree is unchanged; [_dir] are sorts, not terms, and were flattened into it
...
- `_clause` is a facet: [matched, not_matched] listed in roles.json; [_when] left the supertypes list (hidden rules still), since a supertype must be a table-tier term
- roles.json: 9 of 22 table-tier terms are supertypes, 2 facet(s), 64 named node(s), 11 uncategorised (vocabulary 0.1.0)
