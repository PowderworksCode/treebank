...
## MAPPED -- lowered exactly (47)

- imports [rust/2015, rust/keywords-2018] merged additively by the loader: an imported sort gains this module's productions, nothing is overridden -- where tree-sitter's `extends` would flatten and override
...
- `ID = keyword {reject}` became `word: id` plus reserved.global = [async, await, dyn, else, fn, i64, if, let, mut, return, try, while]
- `tokenize: "(),;:{}"`: the reader split template literal runs at these characters, so each is its own token
- rejected words used by no production [async, await, dyn, try]: reserved, and made tokens by a hidden `_reserved_word` rule the start rule reaches only behind a pattern matching nothing, so each is a syntax error wherever it appears
