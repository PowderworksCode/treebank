...
## MAPPED -- lowered exactly (42)

- imports [../rustish/rustish] merged additively by the loader: an imported sort gains this module's productions, nothing is overridden -- where tree-sitter's `extends` would flatten and override
...
- `ID = keyword {reject}` became `word: id` plus reserved.global = [else, fn, i64, if, let, mut, return, while]
