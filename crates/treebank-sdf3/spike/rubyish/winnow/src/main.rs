// GENERATED from rubyish.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn layout(i: &mut In) -> ModalResult<()> {
    ...
    loop {
        ...
        { let s = pos(i); if let Ok(()) = run(((literal("#"), star(none_of(|c: char| matches!(c, '\n'))),).void()).void(), i) { let e = pos(i); i.state.comments.insert(s, (e, "comment")); progressed = true; continue; } }
        if !progressed { break; }
    }
    let after = pos(i);
...
fn lxb_id(i: &mut In) -> ModalResult<()> { run((one_of(|c: char| matches!(c, 'a'..='z' | '_')), star(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))),).void(), i) }
...
fn lxb_nl(i: &mut In) -> ModalResult<()> { run((one_of(|c: char| matches!(c, '\n')), star(alt(((one_of(|c: char| matches!(c, ' ' | '\t' | '\n')),).void(), (literal("#"), star(none_of(|c: char| matches!(c, '\n'))),).void(),))),).void(), i) }
fn lx_nl(i: &mut In) -> ModalResult<Node> {
    let start = pos(i);
    i.state.furthest = i.state.furthest.max(start);
    lxb_nl(i)?;
    let end = pos(i);
    ...
    if REJECT.iter().any(|k| eq_cs(k, text)) { return Err(bt()); }
    token_end(i, end);
    Ok(Node::leaf("nl", start, end))
}
fn lxb_regex(i: &mut In) -> ModalResult<()> { run((literal("/"), none_of(|c: char| matches!(c, '/' | '\n' | ' ' | '\t')), star(none_of(|c: char| matches!(c, '/' | '\n'))), literal("/"),).void(), i) }
fn lx_regex(i: &mut In) -> ModalResult<Node> {
    let start = pos(i);
    i.state.furthest = i.state.furthest.max(start);
    lxb_regex(i)?;
    let end = pos(i);
...
fn c_0_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { let cp = save(i); match (|i: &mut In| -> ModalResult<Vec<Node>> { layout(i)?; Ok(vec![lx_nl(i)?]) })(i) { Ok(v) => v, Err(_) => { restore(i, &cp); Vec::new() } } };
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 5 >= min && block != Some(5) { let cp = save(i); match t_8(i, &left, start) { Ok(n) => { left = n; block = if false { Some(5) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 3 >= min && block != Some(3) { let cp = save(i); match t_12(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 3 >= min && block != Some(3) { let cp = save(i); match t_13(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_14(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_15(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
...
fn c_10_body(i: &mut In) -> ModalResult<Node> {
    ...
    { let s = pos(i);
      let ns: Vec<Node> = vec![lx_id(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("method"), n)); } }
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![r_arg(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("argument"), n)); } }
    if pr[0] && pr[1] && !((col(i, sp[0].1.saturating_sub(1).max(sp[0].0)) as i64) + (1) < (col(i, sp[1].0) as i64)) { return Err(bt()); }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "command", start, end, reach: end, children: ch })
}
...
fn c_5_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![lx_regex(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "exp_regex", start, end, reach: end, children: ch })
}
...
fn c_6_body(i: &mut In) -> ModalResult<Node> {
    ...
    { let s = pos(i);
      ...
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("elements"), n)); } }
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("]"), i)?;  let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "array", start, end, reach: end, children: ch })
}
...
fn t_8(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_8_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_8_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
    ...
    ch.push((Some("receiver"), left.clone()));
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("["), i)?;  let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
    ...
    { let s = pos(i);
      let ns: Vec<Node> = vec![r_exp(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((Some("index"), n)); } }
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("]"), i)?;  let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
...
fn t_12(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_12_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_12_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn t_13(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_13_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_13_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "div", start, end, reach: end, children: ch })
}
fn t_14(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_14_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_14_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn t_15(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_15_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_15_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn r_arg(i: &mut In) -> ModalResult<Node> { r_arg_prec(i, 0) }
fn r_arg_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    layout(i)?;
    let start = pos(i);
    let cp = save(i);
    // Longest match among the primaries: ordered choice would let an
...
fn c_17_body(i: &mut In) -> ModalResult<Node> {
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "splat", start, end, reach: end, children: ch })
}
