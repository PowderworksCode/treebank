...
fn lxb_id(i: &mut In) -> ModalResult<()> {
    run((one_of(|c: char| matches!(c, 'a'..='z' | '_')), star(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))),).void(), i)?;
...
fn lxb_regex(i: &mut In) -> ModalResult<()> {
    run((literal("/"), none_of(|c: char| matches!(c, '/' | '\n' | ' ' | '\t')), star(none_of(|c: char| matches!(c, '/' | '\n'))), literal("/"),).void(), i)?;
    Ok(())
}
