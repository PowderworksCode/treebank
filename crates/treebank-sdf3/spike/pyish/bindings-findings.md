...
- Stmt.Global: `binds(names -> module)`: the locals query dialect cannot name the module scope, so the pattern binds at the nearest scope; bindings.json carries the target
...
- Stmt.Assign: `binds(target -> enclosing as var)` (not SDF3): the `id` under `target` of `assign` is bound in the enclosing scope
- Stmt.Global: `binds(names -> module as var)` (not SDF3): the `id` under `names` of `global` is bound in the module scope
- Stmt.Def: `scope(function)` (not SDF3): `def` delimits a lexical scope
- Stmt.Def: `binds(name -> enclosing as function)` (not SDF3): the `id` under `name` of `def` is bound in the enclosing scope
- Param.Param: `binds(name -> enclosing as parameter)` (not SDF3): the `id` under `name` of `param` is bound in the enclosing scope
