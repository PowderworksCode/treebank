// GENERATED from mini.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn lx_id(i: &mut In) -> ModalResult<Node> {
    ...
    const REJECT: &[&str] = &["else", "fun", "if", "let", "return", "while"];
...
fn c_5_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("fun"), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    layout(i)?;
    { let s = pos(i);
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "fun", start, end, reach: end, children: ch })
}
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 3 >= min && block != Some(3) { let cp = save(i); match t_14(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 3 >= min && block != Some(3) { let cp = save(i); match t_15(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_16(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_17(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 1 >= min && block != Some(1) { let cp = save(i); match t_18(i, &left, start) { Ok(n) => { left = n; block = if true { Some(1) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 1 >= min && block != Some(1) { let cp = save(i); match t_19(i, &left, start) { Ok(n) => { left = n; block = if true { Some(1) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
