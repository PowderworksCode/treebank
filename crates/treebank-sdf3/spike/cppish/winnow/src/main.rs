// GENERATED from cppish.sdf3 by treebank-sdf3's winnow backend. Do not edit.
...
fn lx_id(i: &mut In) -> ModalResult<Node> {
    ...
    const REJECT: &[&str] = &["char", "int"];
    if REJECT.iter().any(|k| eq_cs(k, text)) { return Err(bt()); }
    token_end(i, end);
    Ok(Node::leaf("id", start, end))
}
fn lxb_num(i: &mut In) -> ModalResult<()> { run((plus(one_of(|c: char| matches!(c, '0'..='9'))),).void(), i) }
fn lx_num(i: &mut In) -> ModalResult<Node> {
    let start = pos(i);
    i.state.furthest = i.state.furthest.max(start);
    lxb_num(i)?;
...
fn c_1_body(i: &mut In) -> ModalResult<Node> {
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "decl", start, end, reach: end, children: ch })
}
...
fn c_3_body(i: &mut In) -> ModalResult<Node> {
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "expr_stmt", start, end, reach: end, children: ch })
}
...
fn c_14_body(i: &mut In) -> ModalResult<Node> {
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "template_id", start, end, reach: end, children: ch })
}
...
fn c_4_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("int"), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "int_type", start, end, reach: end, children: ch })
}
...
fn c_5_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = { i.state.furthest = i.state.furthest.max(pos(i)); run(literal("char"), i)?; run(not(one_of(|c: char| matches!(c, 'a'..='z' | 'A'..='Z' | '0'..='9' | '_'))), i)?; let e = pos(i); token_end(i, e); Vec::new() };
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "char_type", start, end, reach: end, children: ch })
}
...
fn r_exp_prec(i: &mut In, min: u32) -> ModalResult<Node> {
    ...
    loop {
        if 3 >= min && block != Some(3) { let cp = save(i); match t_11(i, &left, start) { Ok(n) => { left = n; block = if false { Some(3) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 2 >= min && block != Some(2) { let cp = save(i); match t_10(i, &left, start) { Ok(n) => { left = n; block = if false { Some(2) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 1 >= min && block != Some(1) { let cp = save(i); match t_12(i, &left, start) { Ok(n) => { left = n; block = if false { Some(1) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        if 1 >= min && block != Some(1) { let cp = save(i); match t_13(i, &left, start) { Ok(n) => { left = n; block = if false { Some(1) } else { None }; continue; } Err(_) => restore(i, &cp) } }
        break;
    }
    let _ = block;
    let _ = min;
...
fn c_8_body(i: &mut In) -> ModalResult<Node> {
    ...
    layout(i)?;
    { let s = pos(i);
      let ns: Vec<Node> = vec![lx_num(i)?];
      let e = i.state.last_end.max(s); sp.push((s, e)); pr.push(e > s);
      for n in ns { ch.push((None, n)); } }
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "exp_num", start, end, reach: end, children: ch })
}
...
fn t_11(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_11_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_11_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
...
fn t_10(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    let guard = i.state.offside.len();
    let r = t_10_body(i, left, start);
    i.state.offside.truncate(guard);
    r
}
fn t_10_body(i: &mut In, left: &Node, start: usize) -> ModalResult<Node> {
    #[allow(unused_mut)]
    let mut ch: Vec<(Option<&'static str>, Node)> = Vec::new();
    #[allow(unused_mut)]
    let mut sp: Vec<(usize, usize)> = Vec::new();
    ...
    let _ = (&sp, &pr);
    let end = i.state.last_end.max(start);
    Ok(Node { kind: "shr", start, end, reach: end, children: ch })
}
