...
- Stmt.LetMut: `binds(pattern -> enclosing as var after)` (not SDF3): the `id` under `pattern` of `let_mut` is bound in the enclosing scope, from the end of the node onward
...
- facets from the attributes: _scope = [block, fn, program], _binding = [fn, let, let_mut, param]
