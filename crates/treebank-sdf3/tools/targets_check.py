...
def winnow_binary(root, t):
    """Build the target's winnow crate, if the lowering emitted one."""
    crate = target_dir(root, t) / "winnow"
    if not (crate / "Cargo.toml").exists():
        return None
    from winnow_check import build
    return build(crate)


def parse_winnow(binary, f):
    """(accepted, tree) from the winnow parser, in the same S-expression."""
    code, out = sh([str(binary), str(f)])
    out = out.strip()
...
def main():
    ...
    winnow = {t: winnow_binary(root, t) for t in targets}
    confer = {"agree": 0, "differ": 0}
    for f in files:
        ...
        for t in targets:
            ...
            mark = "✓" if got else "✗"
            if winnow.get(t):
                wgot, wtree = parse_winnow(winnow[t], f)
                same = wgot == got and (not got or flat(wtree) == flat(tree))
                confer["agree" if same else "differ"] += 1
                if not same:
                    failures.append(f"{f.name} on {t}: winnow {'accepted' if wgot else 'rejected'} where tree-sitter {'accepted' if got else 'rejected'}" + ("" if wgot != got else f"; trees differ:\n  tree-sitter: {flat(tree)}\n  winnow:      {flat(wtree)}"))
                    mark += "≠"
                else:
                    mark += "≡"
            if got != want:
    ...
    out.append("GENERATED by tools/targets_check.py. ✓ accepted, ✗ rejected; `≡` the winnow parser of the same target agrees with tree-sitter's verdict and tree, `≠` it does not; `•` the oracle for that exact version agrees with the header; `!` a mismatch; `?` the oracle could not judge.\n")
    ...
    if any(winnow.values()):
        out.append(f"\nThe winnow lowering of each target: {confer['agree']} cells agree with tree-sitter on verdict and tree, {confer['differ']} differ.")
    if trees:
    ...
    print(f"{root.name}: {len(files)} files x {len(targets)} targets = {n} cells, {len(failures)} failures; oracle-checked {checked}; winnow agrees on {confer['agree']} cells, differs on {confer['differ']}")
