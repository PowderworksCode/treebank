//! treebank-hcl — the treebank HCL grammar.
//!
...
//! NOMINALLY (`_callable`, `_binding`, `_scope`, `_clause`, `_comment`,
//! `_identifier`, `_string`), as a list of node types rather than as a
//! supertype in the parse table, plus the nodes deliberately outside the
//! vocabulary and why. It travels inside the published crate so a consumer
//! never has to fetch it separately — the structural terms are already
//! queryable from the parser itself.
...
mod tests {
    ...
    fn ships_its_manifests() {
        ...
        assert_eq!(ledger["language"].as_str(), Some("hcl"));
    }
}
