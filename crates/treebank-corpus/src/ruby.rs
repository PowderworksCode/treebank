...
impl Ecosystem for Ruby {
    ...
    /// most editors claim. That undercounts the language badly: Ruby's
    /// build and configuration files are Ruby programs, not a config
    /// format, and this grammar parses them. Every entry below is present
    /// in the top-1000 gem corpus AND is a recognised Ruby filename
    /// convention (GitHub linguist's Ruby set), and every one was checked
    /// against CRuby before being added — see the ledger for the per-type
    /// counts and verdicts.
    ///
    /// Two neighbours are deliberately NOT here, both of which look like
    /// they belong:
    ///
    /// `.rbs` is the big one — 2,216 files in the earlier corpus, more than
    /// every extension added here combined. RBS is Ruby's *type signature*
    /// language, a separate grammar with its own parser, and CRuby rejects
    /// 35 of 40 sampled files. Taking it would have added two thousand
    /// files that are not Ruby and recorded them as corpus noise, burying
    /// any real gap in the report.
    ///
    /// `Pluginfile` appears once and parses, but one gem's choice of name
    /// is not a convention; it is in no editor's Ruby list, and admitting
    /// names on a single sighting is how a corpus acquires files nobody can
    /// attribute.
    ///
    ...
    /// someone else's source, so a failure there is attributed to the
    /// wrong package and the same code is already in the corpus under its
    /// real owner.
...
fn rank_rubygems(k: usize) -> Result<Vec<RankedCrate>> {
    ...
    while ranked.len() < k {
        ...
        eprintln!(
            "rank: ecosyste.ms rubygems page {page} ({} gems)",
            batch.len()
        );
        for entry in batch {
