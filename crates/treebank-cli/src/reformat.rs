...
pub fn run(
    ...
    let Some(fmt) = treebank_oracle::reformatter_for(lang) else {
        anyhow::bail!(
            "no available formatter for {lang}. Registered adapters use rustfmt for \
             rust, black for python, the TypeScript language service for typescript \
             and javascript, and `zig fmt` for zig. This check needs the language's \
             own formatter — substituting another tool's opinion would make a \
             disagreement mean nothing."
        );
    };
