...
pub fn run(
    ...
    for chunk in entries.chunks(400) {
        ...
        let results: Vec<(bool, bool, Option<(String, Failure)>)> = chunk
            .par_iter()
            .zip(&paths)
            .map(|(f, rel)| -> Result<(bool, bool, Option<(String, Failure)>)> {
                ...
                Ok((true, true, Some((sig, Failure { path: rel.clone(), line, snippet }))))
            })
            .collect::<Result<_>>()?;
        for (r, p, fail) in results {
