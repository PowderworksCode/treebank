//! Skeleton — see `super::skeleton`. Rank 17 in ROADMAP.md.
//! Oracle: `solc --stop-after parsing`. The named risk is version fan-out
//! across 0.4-0.8.
...
impl Lang for Solidity {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Solidity))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Solidity))
    }
