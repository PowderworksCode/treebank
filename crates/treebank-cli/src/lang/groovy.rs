//! Skeleton — see `super::skeleton`. Rank 38 in ROADMAP.md.
//! Oracle: Groovy `AstBuilder`, from the JDK already installed for java.
...
impl Lang for Groovy {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Groovy))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Groovy))
    }
