//! Skeleton — see `super::skeleton`. Rank 15 in ROADMAP.md.
//! Oracle: `rnix` / `nix-instantiate --parse`. The named risk is the
//! corpus: nixpkgs is one repo.
...
impl Lang for Nix {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Nix))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Nix))
    }
