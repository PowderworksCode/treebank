//! Skeleton — see `super::skeleton`. Rank 34 in ROADMAP.md.
//! Oracle: `svelte/compiler` parse. The named risk is embedded-language
//! routing.
...
impl Lang for Svelte {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Svelte))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Svelte))
    }
