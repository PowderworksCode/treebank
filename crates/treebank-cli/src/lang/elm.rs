//! Skeleton — see `super::skeleton`. Rank 39 in ROADMAP.md.
//! Oracle: `elm-syntax`. The named risk is the corpus: a small registry.
...
impl Lang for Elm {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Elm))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Elm))
    }
