//! Skeleton — see `super::skeleton`. Rank 30 in ROADMAP.md.
//! Oracle: `protoc --descriptor_set_out`. The dialect axis is syntax 2 vs 3.
...
impl Lang for Proto {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Proto))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Proto))
    }
