//! Skeleton — see `super::skeleton`. Rank 41 in ROADMAP.md.
//! Oracle: buildkit's parser.
...
impl Lang for Dockerfile {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Dockerfile))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Dockerfile))
    }
