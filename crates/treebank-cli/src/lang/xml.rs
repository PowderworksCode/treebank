//! Skeleton — see `super::skeleton`. Rank 28 in ROADMAP.md.
//! Oracle: libxml2, well-formedness only.
...
impl Lang for Xml {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Xml))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Xml))
    }
