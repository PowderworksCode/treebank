//! Skeleton — see `super::skeleton`. Rank 16 in ROADMAP.md.
//! Oracle: `Parser.ParseFile`, from the .NET already installed for csharp.
//! The named risk is the corpus: PSGallery ships scripts.
...
impl Lang for PowerShell {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Powershell))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Powershell))
    }
