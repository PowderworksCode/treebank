//! Skeleton — see `super::skeleton`. Rank 27 in ROADMAP.md.
//! Oracle: `compiler-libs` Parse. Two grammars, implementation and
//! interface, so `grammar_dirs` will not stay `["."]`.
...
impl Lang for OCaml {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Ocaml))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Ocaml))
    }
