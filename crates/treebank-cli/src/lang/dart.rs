//! Skeleton — see `super::skeleton`. Rank 25 in ROADMAP.md.
//! Oracle: `package:analyzer`, parse-only. The named risk is oracle size
//! (the Dart SDK).
...
impl Lang for Dart {
    ...
    fn rank(&self, _db: &Path, _k: usize) -> Result<Vec<RankedCrate>> {
        Err(not_implemented(LangName::Dart))
    }
    ...
    fn classify(&self, _rel: &Path) -> Option<Option<String>> {
        unreachable!("{}", not_implemented(LangName::Dart))
    }
